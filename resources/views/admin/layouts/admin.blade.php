...
<head>
    ...
    <title>@yield('title', 'Panel Admin') — GTAVerse Admin</title>
    ...
    <style>
        /* Penyesuaian Cropper.js dengan tema gelap */
    ...
    <style>
        /* Kontrol form bertema GTAVerse (berdiri sendiri agar tidak bergantung pada hasil scan Tailwind) */
        .form-label {
            display: block;
...
<body class="bg-zinc-950 font-sans text-zinc-100 antialiased selection:bg-cyan-500/40">
    ...
    @php $unreadSaran = \App\Models\Message::unread()->count(); @endphp
    ...
    <header class="fixed inset-x-0 top-0 z-50 nav-scrolled">
        <nav class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6">
            ...
            <div class="hidden items-center gap-6 text-[11px] font-bold uppercase tracking-[0.18em] md:flex">
                ...
                <a href="{{ route('admin.games.index') }}" class="transition hover:text-cyan-400 {{ request()->routeIs('admin.games.*') ? 'text-cyan-400 neon-text' : 'text-zinc-300' }}">Game</a>
                <a href="{{ route('admin.articles.index') }}" class="transition hover:text-cyan-400 {{ request()->routeIs('admin.articles.*') ? 'text-cyan-400 neon-text' : 'text-zinc-300' }}">Artikel</a>
                <a href="{{ route('admin.characters.index') }}" class="transition hover:text-cyan-400 {{ request()->routeIs('admin.characters.*') ? 'text-cyan-400 neon-text' : 'text-zinc-300' }}">Karakter</a>
                <a href="{{ route('admin.categories.index') }}" class="transition hover:text-cyan-400 {{ request()->routeIs('admin.categories.*') ? 'text-cyan-400 neon-text' : 'text-zinc-300' }}">Kategori</a>
                <a href="{{ route('admin.messages.index') }}" class="relative transition hover:text-cyan-400 {{ request()->routeIs('admin.messages.*') ? 'text-cyan-400 neon-text' : 'text-zinc-300' }}">
                    Saran
                    @if ($unreadSaran > 0)
                        <span class="absolute -right-3.5 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-pink-500 px-1 text-[9px] font-bold text-white">{{ $unreadSaran }}</span>
                    @endif
                </a>

                <span class="h-4 w-px bg-white/15"></span>

                <a href="{{ route('home') }}" target="_blank" class="text-zinc-400 transition hover:text-cyan-400">Lihat Situs ↗</a>
            ...
            <button id="admin-menu-btn" class="text-zinc-200 md:hidden" aria-label="Buka menu">
        ...
        <div id="admin-mobile-menu" class="hidden border-t border-cyan-500/15 bg-zinc-950/95 backdrop-blur-xl md:hidden">
            <div class="flex flex-col gap-1 px-6 py-4 text-sm font-bold uppercase tracking-widest">
                ...
                <a href="{{ route('admin.games.index') }}" class="py-2 text-zinc-200 hover:text-cyan-400">Game</a>
                <a href="{{ route('admin.articles.index') }}" class="py-2 text-zinc-200 hover:text-cyan-400">Artikel</a>
                <a href="{{ route('admin.characters.index') }}" class="py-2 text-zinc-200 hover:text-cyan-400">Karakter</a>
                <a href="{{ route('admin.categories.index') }}" class="py-2 text-zinc-200 hover:text-cyan-400">Kategori</a>
                <a href="{{ route('admin.messages.index') }}" class="py-2 text-zinc-200 hover:text-cyan-400">
                    Saran @if ($unreadSaran > 0)<span class="ml-1 rounded-full bg-pink-500 px-1.5 py-0.5 text-[9px] font-bold text-white">{{ $unreadSaran }}</span>@endif
                </a>
                <a href="{{ route('home') }}" target="_blank" class="py-2 text-zinc-400 hover:text-cyan-400">Lihat Situs ↗</a>
                <form method="POST" action="{{ route('admin.logout') }}">
                    @csrf
                    <button type="submit" class="py-2 uppercase text-pink-400">Keluar</button>
                </form>
            </div>
        </div>
    </header>
    ...
    <main class="mx-auto min-h-screen max-w-7xl px-4 pb-20 pt-24 sm:px-6">
        ...
        @if ($errors->any())
            <div class="mb-6 rounded-xl border border-pink-500/30 bg-pink-500/10 px-5 py-3.5 text-sm text-pink-200">
                <p class="mb-1 font-bold uppercase tracking-widest text-pink-300">Periksa kembali isian Anda:</p>
    ...
    <div id="crop-modal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-zinc-950/85 p-4 backdrop-blur-sm">
        <div class="glass-panel w-full max-w-2xl p-5">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
                <h3 class="font-display text-lg uppercase tracking-wide text-cyan-300">Atur Tampilan Gambar</h3>
                <p class="text-xs text-zinc-500">Geser untuk memindahkan &middot; scroll / tombol untuk zoom</p>
            </div>
            <div class="max-h-[55vh] overflow-hidden rounded-lg bg-zinc-900/80">
                <img id="crop-image" src="" alt="Gambar yang sedang diatur" class="block max-h-[55vh] max-w-full">
            </div>

            {{-- Pilihan rasio potong --}}
            <div class="mt-4 flex flex-wrap items-center gap-2">
                <span class="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Rasio:</span>
                <button type="button" class="crop-ratio btn-admin-sm btn-edit" data-ratio="rec">Disarankan</button>
                <button type="button" class="crop-ratio btn-admin-sm btn-edit" data-ratio="free">Bebas</button>
            ...
            <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div class="flex gap-2">
                    <button type="button" id="crop-zoom-out" class="btn-admin-sm btn-edit" title="Perkecil">&minus; Zoom</button>
                    <button type="button" id="crop-zoom-in" class="btn-admin-sm btn-edit" title="Perbesar">&plus; Zoom</button>
                    <button type="button" id="crop-reset" class="btn-admin-sm btn-edit">Reset</button>
                </div>
                <div class="flex gap-2">
                    <button type="button" id="crop-cancel" class="btn-admin-sm btn-delete">Batal</button>
                    <button type="button" id="crop-apply" class="btn-primary !px-7 !py-2.5">Terapkan</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        /* ==========================================================
           CROP GAMBAR
           - Input file dengan data-aspect membuka modal Cropper.js.
           - Tombol [data-crop-edit] membuka gambar yang SUDAH
             tersimpan agar bisa diatur ulang (zoom, geser, rasio).
           - Rasio bisa diganti: Disarankan / Bebas / 1:1 / 16:9 / 3:4.
           - Hasil potong dimasukkan ke input file; tersimpan saat
             form disubmit.
        ========================================================== */
        (() => {
            ...
            // 1) Gambar baru dipilih dari komputer
            ...
            // 2) Edit gambar yang sudah tersimpan di server
            document.querySelectorAll('[data-crop-edit]').forEach((button) => {
                button.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        const name = decodeURIComponent(button.dataset.src.split('/').pop() || 'gambar.jpg');
                        openCropper(input, new File([blob], name, { type: blob.type || 'image/jpeg' }));
                    } catch (e) {
                        alert('Gambar tidak dapat dimuat untuk diedit. Coba unggah ulang file baru.');
                    } finally {
            ...
            document.getElementById('crop-apply').addEventListener('click', () => {
                ...
                cropper.getCroppedCanvas({ maxWidth: 2560, maxHeight: 2560 }).toBlob((blob) => {
                    ...
                    // Perbarui / buat pratinjau di dekat input
