...
@section('title', 'GTAVerse — Grand Theft Auto News Portal')

@section('content')

{{-- ==================================================================
     HERO IMAGE MAP — slug → static hero image
================================================================== --}}
@php
    $heroImages = [
...
{{-- ==================================================================
     1. HERO SLIDER — cycles automatically & manually between featured games
================================================================== --}}
<section id="hero" class="relative h-screen overflow-hidden scanlines">
    @forelse ($featuredGames as $i => $game)
        <div class="hero-slide absolute inset-0 {{ $i === 0 ? 'is-active' : '' }}" data-index="{{ $i }}">
            {{-- Background: hero image if available, otherwise static image or gradient --}}
            @if ($game->hero_image)
            ...
            <div class="relative z-10 mx-auto flex h-full max-w-7xl flex-col justify-center px-6">
                <span class="hero-el mb-5 w-fit badge-universe">
                    ...
                    @if ($game->status === 'upcoming') · <span class="text-pink-400">Coming Soon</span> @endif
                </span>
                ...
                <div class="hero-el mt-9 flex flex-wrap gap-4">
                    <a href="{{ route('games.show', $game) }}" class="btn-primary">Explore Game</a>
                    <a href="{{ route('articles.index') }}" class="btn-ghost">Read News</a>
                </div>
            </div>
        </div>
    @empty
        <div class="flex h-full items-center justify-center animated-gradient">
            <div class="text-center">
                ...
                <p class="mt-4 text-zinc-500">No featured games yet. Mark a game as featured from the admin panel.</p>
            </div>
        </div>
    @endforelse
    ...
    @if ($featuredGames->count() > 1)
        ...
        <div class="absolute bottom-10 left-1/2 z-20 flex -translate-x-1/2 items-center gap-4">
            <button id="hero-prev" class="hero-arrow" aria-label="Previous slide">&larr;</button>
            ...
            <button id="hero-next" class="hero-arrow" aria-label="Next slide">&rarr;</button>
        </div>
...
{{-- ==================================================================
     2. SCROLL JOURNEY — game panels change as you scroll
================================================================== --}}
@if ($featuredGames->count() > 1)
    <section id="journey" class="relative" style="height: {{ ($featuredGames->count() + 1) * 100 }}vh">
        <div class="sticky top-0 flex h-screen items-center justify-center overflow-hidden">
            ...
            <div class="absolute left-6 top-24 z-20 text-[11px] font-bold uppercase tracking-[0.35em] text-zinc-400">
                Journey Through the <span class="text-gta-accent">GTA</span> Universe<br>
            ...
            @foreach ($featuredGames as $i => $game)
                <article class="journey-panel absolute inset-0 flex items-center justify-center" data-panel="{{ $i }}">
                    ...
                    <div class="relative z-10 mx-auto max-w-5xl px-6 text-center">
                        ...
                        <a href="{{ route('games.show', $game) }}" class="btn-ghost mt-9 inline-flex">View Details</a>
                    </div>
                </article>
            @endforeach
...
@if ($upcoming && $upcoming->release_date?->isFuture())
    <section class="relative border-y border-cyan-500/15 overflow-hidden">
        ...
        <div class="relative z-10 mx-auto flex max-w-7xl flex-col items-center gap-6 px-6 py-14 md:flex-row md:justify-between">
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.3em] text-cyan-400">Countdown</p>
                <h2 class="mt-2 font-display text-3xl uppercase neon-text sm:text-4xl">{{ $upcoming->title }}</h2>
                <p class="mt-1 text-sm text-zinc-400">Releases {{ $upcoming->release_date->format('d F Y') }} &middot; {{ $upcoming->platforms }}</p>
            </div>
            <div id="countdown" class="grid grid-cols-4 gap-3 text-center"
                 data-release="{{ $upcoming->release_date->startOfDay()->toIso8601String() }}">
                @foreach (['Days' => 'cd-days', 'Hours' => 'cd-hours', 'Minutes' => 'cd-mins', 'Seconds' => 'cd-secs'] as $label => $id)
...
{{-- ==================================================================
     4. GAME CATALOG BY UNIVERSE
================================================================== --}}
<section class="mx-auto max-w-7xl px-6 py-24">
    <div class="reveal mb-14 max-w-2xl">
        <p class="text-xs font-bold uppercase tracking-[0.3em] text-cyan-400">Catalog</p>
        <h2 class="mt-3 font-display text-4xl uppercase sm:text-5xl">Two Universes, <span class="text-gta-accent">One Legacy</span></h2>
        <p class="mt-4 text-zinc-400">Explore every Grand Theft Auto entry — from the classic 3D Universe era to the modern HD Universe era.</p>
    </div>

    @foreach ([
        ['label' => '3D Universe', 'desc' => 'The classic 2001–2006 era: Liberty City, Vice City, and San Andreas.', 'games' => $threeDGames],
        ['label' => 'HD Universe', 'desc' => 'The modern era from 2008 onward: realism, multiple protagonists, and Leonida.', 'games' => $hdGames],
    ] as $group)
        <div class="mb-16 last:mb-0">
            <div class="reveal mb-7 flex items-end justify-between border-b border-cyan-500/15 pb-4">
                ...
                <a href="{{ route('games.index', ['universe' => explode(' ', $group['label'])[0]]) }}"
                   class="text-xs font-bold uppercase tracking-widest text-cyan-400 hover:text-cyan-300 transition">View All &rarr;</a>
            </div>
    ...
    <div class="mx-auto max-w-7xl px-6 py-24">
        <div class="reveal mb-12 flex flex-wrap items-end justify-between gap-4">
            <div>
                ...
                <h2 class="mt-3 font-display text-4xl uppercase sm:text-5xl">Latest <span class="text-gta-accent">Articles</span></h2>
            </div>
            <a href="{{ route('articles.index') }}" class="btn-ghost">All Articles</a>
        </div>
               ...
               class="reveal group mb-10 grid overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/70 md:grid-cols-2 transition hover:border-cyan-500/30 hover:shadow-[0_0_30px_rgba(34,211,238,0.08)]">
                ...
                <div class="flex flex-col justify-center p-8">
                    <p class="text-xs text-zinc-500">
                        {{ $headline->category?->name ?? 'General' }} &middot; {{ $headline->published_at->format('d M Y') }}
                    </p>
                    ...
                    <span class="mt-6 text-xs font-bold uppercase tracking-widest text-cyan-400">Read More &rarr;</span>
                </div>
            </a>
        @endif
...
<section class="mx-auto max-w-7xl px-6 py-24">
    <div class="reveal mb-12 flex flex-wrap items-end justify-between gap-4">
        <div>
            ...
            <h2 class="mt-3 font-display text-4xl uppercase sm:text-5xl">The <span class="text-gta-accent">Protagonists</span></h2>
            <p class="mt-3 max-w-xl text-zinc-400">Meet the leading characters of every era — from the silent Claude to the duo of Jason &amp; Lucia.</p>
        </div>
        <a href="{{ route('characters.index') }}" class="btn-ghost">All Characters</a>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    /* ---------------------------------------------------------
       HERO SLIDER — auto-advances every 6 seconds + manual nav
    --------------------------------------------------------- */
    ...
    /* ---------------------------------------------------------
       SCROLL JOURNEY — game panels follow the scroll position
    --------------------------------------------------------- */
